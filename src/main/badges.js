...
export default async (React) => {
  const UserProfileBadgeList = await webpack.getModuleByDisplayNameAsync("UserProfileBadgeList")
  ...
  function Badge({ text, children, onClick = () => {}, className }) {
    return React.createElement(Tooltip, {
      allowOverflow: false,
      children: (props) => React.createElement(Clickable, {
        ...props,
        ...
        children: React.createElement(children, { className })
      }),
      color: "primary",
      forceOpen: false,
      hide: false,
  ...
  window.DrApi.badges = {
    add: ({ userId, id, children, onClick, text }) => {
      ...
      b.set(id, {
        children: typeof children === "string" ? ({ className }) => React.createElement("img", {
          className,
          src: children
        }) : children,
        onClick,
  ...
  function callback(that, [ props ], res) {
    const className = `${classes[`profileBadge${UserProfileBadgeList.BadgeSizes[props.size].replace("SIZE_", "")}`]} ${classes.desaturate}`
    
    if (!badges[props.user.id]) return

    Array.from(badges[props.user.id].values(), (badge) => {
      res.props.children.push(React.createElement(Badge, {
        ...badge,
        key: `DrBadge_${btoa(badge.text)}`,
        className
      }))
    })
  }
