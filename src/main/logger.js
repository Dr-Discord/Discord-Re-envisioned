...
void async function() {
  const UserSettingsProtoStore = await webpack.getModuleByPropsAsync("settings", "frecencyWithoutFetchingLatest")
  isDark = Boolean(UserSettingsProtoStore.settings.appearance)
  UserSettingsProtoStore.addChangeListener(() => isDark = Boolean(UserSettingsProtoStore.settings.appearance))
}()
...
export default new class rawLogger {
  log(title, ...messages) {
    console.log(
      `%cDR%c${title}%c\n`, 
      `background-image:url(data:image/svg+xml;base64,${getIcon(isDark ? "#202124" : "#fff")}); color: transparent; background-size: 24px; background-repeat: no-repeat; padding: 5px; background-color: #F52590; border-radius: 4px`, 
      `background: #F52590; margin-left: 5px; margin-bottom: 9px; padding: 2px; border-radius: 4px; color: ${isDark ? "#202124" : "#fff"}`, 
      "padding-left: 3px",
      ...messages
    )
  }
